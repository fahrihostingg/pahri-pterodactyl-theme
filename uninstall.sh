...
run_local_uninstaller() {
    local root="$1"
    [[ -f "$root/scripts/uninstall-local.sh" ]] || die "scripts/uninstall-local.sh tidak dijumpai."
    bash "$root/scripts/uninstall-local.sh"
}

if [[ -n "$SELF_DIR" && -f "$SELF_DIR/scripts/uninstall-local.sh" ]]; then
    run_local_uninstaller "$SELF_DIR"
    exit $?
fi
...
log "Memuat turun skrip rollback ${THEME_REPO}@${THEME_REF}..."
if command -v curl >/dev/null 2>&1; then
    ...
    die "curl atau wget diperlukan untuk rollback melalui GitHub."
fi
...
run_local_uninstaller "$SOURCE_DIR"
