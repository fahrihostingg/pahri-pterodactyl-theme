...
const Scene = styled.div`
    ...
    &::before {
        ...
        mask-image: radial-gradient(circle at 50% 35%, #000 10%, transparent 78%);
    }
    ...
    &::after {
        ...
        animation: ${scan} calc(var(--pahri-motion-duration, 18s) * 1.7) linear infinite;
...
const CursorAura = styled.span`
    ...
    width: 520px;
    height: 520px;
    left: calc(var(--pahri-cursor-x, 50vw) - 260px);
    top: calc(var(--pahri-cursor-y, 40vh) - 260px);
    border-radius: 50%;
    opacity: .22;
    filter: blur(36px);
    background: radial-gradient(circle, color-mix(in srgb, var(--pahri-accent) 24%, transparent), transparent 67%);
...
const Shape = styled.span<{ $size: number; $top: string; $left: string; $delay: string }>`
    ...
    background:
        ...
        radial-gradient(circle at 20% 10%, color-mix(in srgb, var(--pahri-accent) 18%, transparent), transparent 48%);
    box-shadow: inset 0 1px rgba(255,255,255,.07), 0 35px 90px rgba(0,0,0,.22);
    transform-style: preserve-3d;
    animation: ${float} var(--pahri-motion-duration, 18s) ease-in-out infinite;
...
const Particle = styled.i<{ $left: number; $delay: number; $duration: number; $size: number }>`
    display: var(--pahri-particles-display, block);
    position: absolute;
    left: ${props => props.$left}%;
    bottom: -20px;
    width: ${props => props.$size}px;
    height: ${props => props.$size}px;
    border-radius: 50%;
    background: ${props => (props.$left % 2 ? 'var(--pahri-accent)' : 'var(--pahri-accent-secondary)')};
    box-shadow: 0 0 13px currentColor;
    opacity: 0;
    animation: ${particleRise} ${props => props.$duration}s linear infinite;
