...
for relative in "${REPLACE_FILES[@]}"; do [[ -f "$PANEL_DIR/$relative" ]] || die "Source panel tiada: $relative"; done
for relative in "${ALL_FILES[@]}"; do [[ -f "$SOURCE_DIR/$relative" ]] || die "Source tema tiada: $relative"; done

ensure_node() {
    local current_major=0
    if command -v node >/dev/null 2>&1; then
        current_major="$(node -p "Number(process.versions.node.split('.')[0])" 2>/dev/null || echo 0)"
        if (( current_major >= 22 )); then log "Node.js sedia ada dikesan: $(node -v)"; return; fi
...
ensure_yarn() {
    if command -v yarn >/dev/null 2>&1; then log "Yarn sedia ada dikesan: $(yarn --version)"; return; fi
    log "Menyediakan Yarn Classic 1.22.22..."
    if command -v corepack >/dev/null 2>&1; then corepack enable || true; corepack prepare yarn@1.22.22 --activate || true; fi
    if ! command -v yarn >/dev/null 2>&1; then command -v npm >/dev/null 2>&1 || die "npm tidak dijumpai selepas pemasangan Node.js."; npm install --global yarn@1.22.22; hash -r; fi
    command -v yarn >/dev/null 2>&1 || die "Yarn gagal disediakan."
    ok "Yarn $(yarn --version) berjaya disediakan."
}

copy_theme_files() { local from="$1"; local to="$2"; for relative in "${ALL_FILES[@]}"; do [[ -f "$from/$relative" ]] || continue; install -D -m 0644 "$from/$relative" "$to/$relative"; done; }
backup_current_files() { local target="$1"; mkdir -p "$target"; for relative in "${REPLACE_FILES[@]}"; do install -D -m 0644 "$PANEL_DIR/$relative" "$target/$relative"; done; for relative in "${EXTRA_FILES[@]}"; do [[ -f "$PANEL_DIR/$relative" ]] && install -D -m 0644 "$PANEL_DIR/$relative" "$target/$relative"; done; }
build_panel() { cd "$PANEL_DIR"; export NODE_OPTIONS="${NODE_OPTIONS:-} --max_old_space_size=4096"; yarn install --frozen-lockfile --network-timeout 600000; yarn build:production; php artisan optimize:clear >/dev/null; }
restore_run_backup() { [[ -n "$RUN_BACKUP" && -d "$RUN_BACKUP" ]] || return 0; warn "Build gagal. Memulihkan source sebelum pemasangan..."; for relative in "${REPLACE_FILES[@]}"; do install -D -m 0644 "$RUN_BACKUP/$relative" "$PANEL_DIR/$relative"; done; for relative in "${EXTRA_FILES[@]}"; do [[ -f "$RUN_BACKUP/$relative" ]] && install -D -m 0644 "$RUN_BACKUP/$relative" "$PANEL_DIR/$relative" || rm -f "$PANEL_DIR/$relative"; done; (build_panel) || warn "Source dipulihkan tetapi rebuild asal gagal. Jalankan yarn build:production secara manual."; }
on_error() { local code=$?; [[ $COMPLETED -eq 0 ]] && restore_run_backup; exit "$code"; }
trap on_error ERR
...
if [[ ! -f "$STATE_FILE" ]]; then
    ...
    for relative in "${REPLACE_FILES[@]}"; do install -D -m 0644 "$PANEL_DIR/$relative" "$ORIGINAL_BACKUP/$relative"; done
