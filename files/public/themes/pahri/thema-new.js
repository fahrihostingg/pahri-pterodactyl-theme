(() => {
    ...
    const toggleMarkup = (name, title, description, enabled) => `
        <div class="pahri-toggle-control pahri-toggle-wide">
            ...
            <button type="button" class="pahri-toggle-button ${enabled ? 'is-active' : ''}" aria-pressed="${enabled ? 'true' : 'false'}">
                ...
                <span class="pahri-toggle-copy">
                    <strong>${escapeHtml(title)}</strong>
                    <small>${escapeHtml(description)} <b class="pahri-toggle-state">${enabled ? 'ON' : 'OFF'}</b></small>
                </span>
            </button>
        </div>`;
    ...
    const injectDockStudio = async () => {
        ...
        studio.innerHTML = `
            ...
            <div class="row">
                <div class="form-group col-md-4"><label>Maintenance Badge</label><input type="text" name="maintenance_badge" class="form-control" maxlength="40" value="${escapeHtml(maintenance.badge)}" placeholder="Maintenance Mode"></div>
                <div class="form-group col-md-8"><label>Maintenance Title</label><input type="text" name="maintenance_title" class="form-control" maxlength="120" value="${escapeHtml(maintenance.title)}" placeholder="Panel sedang maintenance"></div>
            </div>
            ...
            <p>User masih boleh akses panel, tetapi UI akan nampak rosak, glitch, terminal, red alert dan macam kena hack. Ini visual simulation sahaja.</p>
            ${toggleMarkup('security_drill_enabled', 'Fake Hacking Theme', 'Tema hancur visual tanpa block akses', Boolean(drill.enabled))}
            <div class="row">
                <div class="form-group col-md-4"><label>Hack Badge</label><input type="text" name="security_drill_badge" class="form-control" maxlength="40" value="${escapeHtml(drill.badge)}"></div>
                <div class="form-group col-md-8"><label>Hack Title</label><input type="text" name="security_drill_title" class="form-control" maxlength="120" value="${escapeHtml(drill.title)}"></div>
            </div>
            ...
            <p>Kawal floating dock bawah panel, support bubble dan spotlight card.</p>
            ${toggleMarkup('dock_active', 'Floating Nexus Dock', 'Quick actions dan support bubble', Boolean(dock.active))}
            <div class="row">
                <div class="form-group col-md-5"><label>Support Bubble Label</label><input type="text" name="dock_support_label" class="form-control" maxlength="32" value="${escapeHtml(dock.support_label)}" placeholder="Support"></div>
                <div class="form-group col-md-7"><label>Support Bubble URL</label><input type="text" name="dock_support_url" class="form-control" maxlength="500" value="${escapeHtml(dock.support_url)}" placeholder="/account atau https://wa.me/..."></div>
            </div>
            <div class="row">
                <div class="form-group col-md-5"><label>Spotlight Title</label><input type="text" name="dock_spotlight_title" class="form-control" maxlength="80" value="${escapeHtml(dock.spotlight_title)}"></div>
                <div class="form-group col-md-7"><label>Spotlight Message</label><input type="text" name="dock_spotlight_message" class="form-control" maxlength="220" value="${escapeHtml(dock.spotlight_message)}"></div>
            </div>`;
